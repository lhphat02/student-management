...
export default function ActionStudentModal({ close, ID, hoten, gioitinh, ngaysinh, diachi, email}) {
        ...
        <div className="h-[22rem]">
            ...
            <form onSubmit={handleSubmit} className="w-[40rem] grid grid-cols-2 gap-x-2 gap-y-12 items-center ml-4">
                ...
                <div className="relative translate-x-[20rem] translate-y-[1rem]">
                    <button className="right-0 bottom-0 -translate-x-40 absolute  bg-[#f59e0b] text-white p-2 rounded-lg" onClick={() => { DeleteStudent(ID) }}>Delete</button>
                    <button className="right-0 bottom-0 absolute -translate-x-8 bg-[#374151] text-white p-2 rounded-lg" type="submit" onClick={() => { updateStudent(ID) }}>Save Changes</button>
                </div>
            </form>
        </div>
    )
